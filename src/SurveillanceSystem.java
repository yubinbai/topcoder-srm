...
public class SurveillanceSystem
{
	...
	{
		// transform container layout to numbers
        int n = containers.length();
        int[] layout = new int[n - L + 1];
        for (int i = 0; i < n; i++) {
            for (int j = i; j < i + L && i + L <= n; j++) {
                layout[i] += containers.charAt(j) == 'X' ? 1 : 0;
            }
        }
        ...
        for (int ct = 0; ct <= L; ct++) {
            int z = count(layout, ct);
            int zz = count(reports, ct);
            if (z == zz) {
                // match
    ...
    private int count(int[] layout, int i) {
        int ret = 0;
        for (int ii : layout) {
            if (i == ii) ret++;
        }
