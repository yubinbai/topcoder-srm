...
public class TheFootballDivTwo {
    class Team implements Comparable<Team> {
        ...
        public Team(int score, int number, int matchLeft) {
            this.score = score;
        ...
        public int compareTo(Team o) {
            if (this.matchLeft == o.matchLeft) {
                return Integer.compare(this.score, o.score);
            } else {
                return Integer.compare(this.matchLeft, o.matchLeft);
            }
        }
    }
    ...
    public int find(int yourScore, int[] scores, int[] numberOfTeams) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            t.add(new Team(scores[i], numberOfTeams[i], 2));
        }
        Collections.sort(t);
        yourScore += 6;
        ...
        for (int i = 0; i < n; i++) {
            if (ptToGive == 0) break;
            Team curr = t.get(i);
            if (curr.score + 6 <= yourScore) {
                if (ptToGive >= curr.number * 6) {
                    ptToGive -= curr.number * 6;
                    curr.score += 6;
                    curr.matchLeft = 0;
                } else {
                    int c = ptToGive / 6;
                    t.add(new Team(curr.score, curr.number - c, curr.matchLeft));
                    curr.number = c;
                    curr.score += 6;
                    curr.matchLeft = 0;
                    ptToGive = 0;
                }
            } else if (curr.score + 3 <= yourScore) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            Team curr = t.get(i);
            if (ptToGive > 0 && curr.matchLeft > 0) {
                if (ptToGive >= curr.number * curr.matchLeft * 3) {
                    ptToGive -= curr.number * curr.matchLeft * 3;
                    curr.score += curr.matchLeft * 3;
                    curr.matchLeft = 0;
                } else if (curr.matchLeft == 1) {
                    int c = ptToGive / 3;
                    t.add(new Team(curr.score, curr.number - c, 1));
                    curr.number = c;
                    curr.score += 3;
                    curr.matchLeft -= 1;
                    ptToGive = 0;
                } else if (curr.matchLeft == 2) {
                    int c = ptToGive / 6;
                    if (c > 0) {
                        t.add(new Team(curr.score + 6, c, 0));
                        curr.number = curr.number - c;
                        ptToGive = ptToGive - c * 6;
                    }
                    if (ptToGive > 0) {
                        c = ptToGive / 3;
                        t.add(new Team(curr.score + 3, c, 1));
                        curr.number = curr.number - c;
                        ptToGive = 0;
                    }
                }
