...
public class CandidatesSelectionEasy {
    class Unit implements Comparable<Unit> {
        ...
        int origin;
        int skill;
        public Unit (String s, int origin, int skill) {
            this.s = s;
            this.origin = origin;
            this.skill = skill;
        }
        public int compareTo(Unit other) {
            return Character.compare(this.s.charAt(skill), other.s.charAt(skill));
        }
    }
    public int[] sort(String[] score, int x) {
        Unit[] all = new Unit[score.length];
        for (int i = 0; i < score.length; i++) {
            all[i] = new Unit(score[i], i, x);
        }
        Arrays.sort(all);
