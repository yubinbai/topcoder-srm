...
public class SortishDiv2 {
	public int ways(int sortedness, int[] seq) {
		...
		for (int i = 1; i <= n; ++i) {
			if (!seen[i]) fill.add(i);
		}
		int ans = 0;
		do {
			for (int i = 0; i < pos.size(); ++i) {
				seq[pos.get(i)] = fill.get(i);
			}
			int curr = 0;
			for (int i = 0; i < n; ++i) {
				for (int j = i + 1; j < n; ++j) {
			...
			if (curr == sortedness) ans++;
		} while (nextPermutation(fill));
		return ans;
	}
	// get the next permutation in place
	public static boolean nextPermutation(ArrayList<Integer> num) {
		int size = num.size();
