...
public class Subsets {
    ...
    public int findSubset(int[] numbers) {
        ...
        go(1, numbers[0], numbers[0]);
        return ans;
    }
    private void go(int index, int csum, int cpro) {
        if (index == n) return;
        for (int i = index; i < n; ++i) {
            if ((index == i || numbers[i] != numbers[i - 1]) && csum + numbers[i] > cpro * numbers[i]) {
                ans++;
                go(i + 1, csum + numbers[i], cpro * numbers[i]);
            }
        }
    }
